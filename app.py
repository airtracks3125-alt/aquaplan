...
# 認証成功後のメニュー作成画面
    ...
    if submit_button:
        ...
        # 生成結果をセッションに保存（印刷シートのトグル切替時に消えないようにする）
    ...
    if "menu_data" in st.session_state:
        ...
        if show_print_sheet:
            ...
            @media print {
                ...
                /* Streamlitの全UIコントロールを印刷非表示 */
                header, footer, [data-testid="stHeader"], [data-testid="stSidebar"],
                [data-testid="stForm"], [data-testid="stMetric"], .stButton,
                [data-testid="stTextArea"], [data-testid="stWidgetLabel"],
                .stToggle, hr, .element-container:has(.stToggle) {
            ...
            for block in result["menu_blocks"].values():
                ...
                <tr>
                    ...
                    <td style="width: 20%; text-align: center;">
                        ...
                        <span style="font-size:12px; color:#555;">({block['duration_str']})</span>
                    </td>
                    <td style="width: 50%;">
            ...
            <div class="swim-sheet-wrapper">
                ...
                <div class="swim-sheet-meta">
                    ...
                    <div class="swim-sheet-meta-item"><strong>練習時間:</strong> {summary['start_time']} 〜 {summary['end_time']} ({summary['total_available_str']})</div>
            ...
            st.info("💡 **印刷方法:** ブラウザの印刷機能（PC: `Ctrl + P` / `Cmd + P` 、スマホ: ブラウザメニューの「印刷」）を実行すると、上記シートのみがA4縦1枚で印刷されます。")
