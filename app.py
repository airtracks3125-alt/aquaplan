...
# --- 【B】メイン画面 ---
    ...
    with col_p2:
        if st.button("📄 SWIM SHEET を一括印刷", type="primary", use_container_width=True):
            # JavaScriptでブラウザの印刷ダイアログを呼び出し
            components.html(
            ...
            with lane_cols[idx]:
                ...
                if st.button("このセットを末尾に追加", key=f"add_pre_{lane_name}"):
                    ...
                    if target_item:
                        ...
                        recalculate_timeline(menu_items, start_datetime)
                        st.toast(
                            f"{lane_name} にプリセットを追加しました！"
                        )
                        st.rerun()

                # --- 自作プリセット保存フォーム ---
                with st.expander("⭐ 現在のセットをプリセットに登録"):
                    new_preset_name = st.text_input(
                        "プリセット名",
                        value=f"自作セット_{lane_name}",
                        key=f"new_pre_name_{lane_name}",
                    )
                    if st.button("登録する", key=f"save_pre_{lane_name}"):
                        if menu_items:
                            # Mainまたは末尾のメニューを代表として保存
                            sample_item = dict(menu_items[-1])
                            st.session_state["custom_presets"].append(
                                {"name": new_preset_name, "item": sample_item}
                            )
                            st.success(
                                f"「{new_preset_name}」をプリセットに追加しました！"
                            )
                            st.rerun()
